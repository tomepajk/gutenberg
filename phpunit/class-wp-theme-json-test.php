...
class WP_Theme_JSON_Gutenberg_Test extends WP_UnitTestCase {
	...
	public function test_get_top_level_background_image_styles() {
		...
		$this->assertSame( $expected_styles, $theme_json->get_stylesheet(), 'Styles returned from "::get_stylesheet()" with default background-size does not match expectations' );
		...
		$this->assertSame( $expected_styles, $theme_json->get_stylesheet(), 'Styles returned from "::get_stylesheet()" with set background size does not match expectations' );
	}
